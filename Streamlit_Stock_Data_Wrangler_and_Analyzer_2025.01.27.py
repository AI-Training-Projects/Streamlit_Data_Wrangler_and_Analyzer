...
def save_dataset(df, filename):
    try:
        ...
        st.success(f"Successfully saved dataset to {filename}")
    except Exception as e:
        logging.error(f"Error saving dataset to {filename}: {str(e)}")
        st.error(f"Error saving dataset to {filename}: {str(e)}")
...
def main():
    ...
    if st.button("Preview Data"):
        ...
        preview_data(df_preview, datatype_map)
    ...
    if st.button("Save Cleaned Dataset"):
        ...
        save_dataset(df_cleaned, csv_filename)
